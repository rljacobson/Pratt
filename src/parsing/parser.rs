...
const OPERATOR_FILE_PATH: &str = "resources/operators.csv";

enum OperatorTableKind {
  Left,
  Null,
  Other
}
...
pub struct Parser<'t> {
  lexer: Lexer<'t>,
  l_operator_table: OperatorTable,
  n_operator_table: OperatorTable,
}

impl<'t> Parser<'t> {
  /// Creates a parser using default operators from `OPERATOR_FILE_PATH`.
  pub fn new(input: &'t str) -> Self {
    Self::with_operator_file(input, OPERATOR_FILE_PATH)
  }

  pub fn with_operator_file(input: &'t str, file_path: &str) -> Self {
    // ToDo: Cache the tokens and operators read from file (make static).
    let (op_tables, tokens): (OperatorTables, Vec<String>) = get_operator_tables_from_file(file_path);
    Parser {
      lexer: Lexer::new(input, tokens),
      l_operator_table: op_tables.left,
      n_operator_table: op_tables.null,
    }
  }

  pub fn parse(&mut self) -> Result<Atom, ()> {
  ...
  fn parse_expression(
    ...
    let (mut current_root, operator): (Atom, Operator) =
        self.lookup_token(&token, OperatorTableKind::Null)?;
    log(
      Channel::Debug,
      5,
    ...
    self.null_denotation(&mut current_root, &operator).map_err(|_| token.clone())?;
    ...
    loop {
      ...
      let (mut new_root, operator): (Atom, Operator) =
          match self.lookup_token(&token, OperatorTableKind::Left) {
      ...
      */
      ...
      self.left_denotation(&mut new_root, &operator)
  ...
  fn null_denotation(&mut self, expression: &mut Atom, operator: &Operator) -> Result<(), ()> {
  ...
  /// otherwise.
  fn expect(&mut self, expected_token: IString) -> Result<(), ()> {
    match self.consume_token() {
      ...
      anything_else => {
        log(Channel::Error, 1, format!("Expected: {}, Found: {}", expected_token, anything_else).as_str());
        Err(())
      }
  ...
  fn optional_expect(&mut self, expected_token: IString) -> bool {
  ...
  /// `expression` as children. The `operator` holds information about `expression`.
  fn left_denotation(&mut self, root: &mut Atom, operator: &Operator) -> Result<(), ()>{
    ...
    match operator {
      ...
      Operator::OptionalTernary{ o_token, ..} => {
        ...
        if self.optional_expect(o_token.clone()) {
  ...
  /// `OperatorRecord`.
  fn lookup_token(&self, token: &Token, table_kind: OperatorTableKind) -> Result<(Atom, Operator), Token> {
    let table = match table_kind {
      OperatorTableKind::Left => {
        &self.l_operator_table
      }
      OperatorTableKind::Null => {
        &self.n_operator_table
      }
      OperatorTableKind::Other => {
        unreachable!("Tried to look up in the o-token table. This is a bug.");
      }
    };
